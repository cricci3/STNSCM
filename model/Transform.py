...
class Transform(nn.Module):
    def __init__(self, time_channels, hidden_channels,
        ...
        # Separately process different periodic features
    ...
    def forward(self, encoder_hidden, x_time, target_time):
        """
        Args:
            encoder_hidden: Tensor of shape [batch_size, rnn_layer, node_num, num_for_predict, hidden_channels]
            x_time: Tensor of shape [batch_size, node_num, 18, 67]
            target_time: Tensor of shape [batch_size, node_num, 2, 67]
        Returns:
            Processed tensor after temporal attention mechanism
        """

        batch_size, rnn_layer, node_num, num_for_predict, hidden_channels = encoder_hidden.shape

        # According to periodicity, merge features along the dimension
        ...
        query = self.FC_q(target_time)          # Embedding future timestamps to form queries — find historical patterns similar to future states
        key = self.FC_k(x_time)                 # Historical time embeddings to serve as keys
        value = self.FC_v(encoder_hidden)       # Historical flow features to be weighted for future transformation
        ...
        value = torch.cat(torch.split(value, self.d, dim=-1), dim=0)                  # [K * batch_size, RNN_layer, num_nodes, num_for_predict, d]

        # Attention over the time dimension — need to swap spatial and temporal axes
        ...
        # Expand attention across RNN layers
        ...
        X = torch.cat(torch.split(X, batch_size, dim=0), dim=-1)  # [batch_size, rnn_layer, node_num, num_pred, d * K]

        # Merge prediction steps
        X = self.FC(X.reshape(batch_size, rnn_layer, node_num, -1))  # [batch_size, rnn_layer, node_num, hidden_channels]
