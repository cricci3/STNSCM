...
def load_dataset(dataset_dir,
    ...
    all_data = {
        'train': {
            ...
            'pos': cat_data['train_pos'],  # Timestamps for target prediction positions
        },
        'val': {
            ...
            'pos': cat_data['val_pos'],  # Timestamps for target prediction positions
        },
        'test': {
            ...
            'pos': cat_data['test_pos'],  # Timestamps for target prediction positions
        },
        'time_feature_index': cat_data['time_feature_index'].item(),
        'time_weather_data': cat_data['time_weather_data'],
    }

    scaler = StandardScaler(mean=all_data['train']['x'].mean(),  # 0th dimension: speed features, 1st dimension: time features
    ...
    # Curriculum learning: since the decoder sometimes uses the ground-truth labels as input, we normalize train_target too
    ...
    train_dataset = traffic_demand_prediction_dataset(
        all_data['train']['x'],
    ...
    val_dataset = traffic_demand_prediction_dataset(
        all_data['val']['x'],
        all_data['val']['x_time'],
        all_data['val']['target'],
    ...
    test_dataset = traffic_demand_prediction_dataset(
        all_data['test']['x'],
    ...
    dataloader = {}
    dataloader['train'] = DataLoader(dataset=train_dataset, shuffle=True, batch_size=train_batch_size)  # num_workers=16 (can be set)
    ...
    logger.info('\n')
    logger.info('scaler.mean : {}, scaler.std : {}'.format(scaler.mean, scaler.std))
...
class traffic_demand_prediction_dataset(Dataset):
    def __init__(self, x, x_time, target, target_time, pos, target_cl=None):
        # One-hot encode the input time features
